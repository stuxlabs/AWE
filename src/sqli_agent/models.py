...
class InjectionPoint:
    ...
    parameter: str
    location: str  # query, post, cookie, header, body, graphql
    original_value: str
    parameter_type: str  # numeric, string, etc.
    confidence: int = 0
    notes: str = ""
    endpoint_url: str = ""  # Full endpoint URL (for POST body parameters discovered from JS)
    graphql_field: str = ""  # GraphQL query field name (e.g., "jobs" for jobs(jobType: "...")
    graphql_return_fields: List[str] = field(default_factory=list)  # Fields returned by GraphQL query
